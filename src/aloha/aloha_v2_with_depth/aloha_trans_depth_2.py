...
class DataProcessor(object):
    ...
    def create_dataset(self):
        ...
        if int(self.is_depth) == 1:
            ...
            for camera_dict in self.config.rgb_names_2:
                rgb_name = next(iter(camera_dict))  # 获取字典的键（如 'cam_high'）
                features[rgb_name] = camera_dict[rgb_name]  # 赋值配置
        elif int(self.is_depth) == 3:
            features['observation.images.depth_cam_high'] = self.config.rgb_names_depth_2['observation.images.depth_cam_high']
            for camera_dict in self.config.rgb_names_3:
                rgb_name = next(iter(camera_dict))  # 获取字典的键（如 'cam_high'）
                features[rgb_name] = camera_dict[rgb_name]  # 赋值配置
    ...
    def read_csv_strange(self,path):
        df = pd.read_csv(path)
        # 检查是否存在 'trans' 列
    ...
    def download_from_nas(self, nas_path, an_path):
        ...
        local_path = os.path.join(self.config.hdf5_root, relative_nas_path)
    ...
    def parse_annotation(self):
        ...
        new_annotation = []
        new_device = []
        task_name_strange = self.task_name + '.csv'
        csv_strange_path = os.path.join(self.config.source_data_csv_path,task_name_strange)
        nested_list_strange = self.read_csv_strange(csv_strange_path)
            ...
            for source_idx,hdf5_file in enumerate(data):
                ...
                for item in nested_list_strange:
                    if episode_path in str(item[1]):
                        nas_path_1 = item[0]
                        break
                nas_path,local_path = self.download_from_nas(nas_path_1,episode_path)
        ...
        try:
            ...
            for i in tqdm(range(index), desc=f'Adding episode {episode_path}'):
                ...
                if int(self.is_depth) == 1:
                    ...
                    frame = {next(iter(rgb_name)): trans_img(raw_images[next(iter(rgb_name))][i]) for rgb_name in self.config.rgb_names_3}
                    frame['observation.images.depth_cam_high'] = raw_depth[i].reshape((768, 1024,1))  
                if i == index - 1:
