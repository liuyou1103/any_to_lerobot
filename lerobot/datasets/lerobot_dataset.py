    ...
    def add_frame(self, frame: dict, task: str, timestamp: float | None = None) -> None:
        ...
        for key in frame:
            ...
            if self.features[key]["dtype"] in ["image", "video"]:
                ...
                if self.features[key].get("is_depth", False):
                    # 确保深度图是2D数组 (H, W)
                    if isinstance(frame[key], torch.Tensor):
                        frame[key].cpu().numpy().astype(np.uint16)
                    else:
                        depth_img = frame[key]
                    if depth_img.ndim == 3 and depth_img.shape[-1] == 1:
                        depth_img = depth_img.squeeze(-1)
                    if depth_img.ndim != 2:
                        raise ValueError(f"Depth image should be 2D (H,W), got shape {depth_img.shape}")
                    # 检查是否是 uint16
                    if depth_img.dtype == np.uint16:
                        depth_pil = PIL.Image.fromarray(depth_img, mode='I;16')  # 保留 16 位精度
                        depth_pil.save(img_path, format='PNG')  # 必须用 PNG 格式
                    else:
                        # 如果不是 uint16，则按 8 位处理（兼容旧逻辑）
                        depth_pil = PIL.Image.fromarray(depth_img, mode='L')
                        depth_pil.save(img_path)
                else:
...
class MultiLeRobotDataset(torch.utils.data.Dataset):
    ...
    def __repr__(self):
        return (
            ...
            f"  Type: {'video (.avi)' if self.video else 'image (.png)'},\n"
